...
const WHATSAPP_DEBUG =
  (typeof process !== "undefined" &&
    ...
    ["1", "true", "yes", "on"].includes(
      process.env.WHATSAPP_DEBUG.toLowerCase(),
    )) ||
  true; // FORCE DEBUG ON
...
// Build LID → phone reverse map from session files (lid-mapping-{LID}_reverse.json)
function buildLidMap() {
  const map = {};
  try {
    for (const f of readdirSync(SESSION_DIR)) {
      // Match reverse mapping files: lid-mapping-{LID}_reverse.json
    ...
    if (WHATSAPP_DEBUG)
      console.error("[bridge] Error building LID map:", err.message);
  }
  if (WHATSAPP_DEBUG) console.log("[bridge] LID map:", map);
  return map;
}
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      const event = {
        messageId: msg.key.id,
        chatId,
        senderId: isGroup ? `${resolvedSenderNumber}@s.whatsapp.net` : senderId, // Send resolved phone for groups, keep LID for DMs
        senderName: msg.pushName || resolvedSenderNumber,
        chatName: isGroup
          ? chatId.split("@")[0]
          : msg.pushName || resolvedSenderNumber,
        isGroup,
        body,
        hasMedia,
        mediaType,
